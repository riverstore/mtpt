...
define('PM_SYSBOX',-2); // GET value for sys box
// Determine action
...
// Get Mailbox Name
if ($mailbox != PM_INBOX && $mailbox != PM_SENTBOX && $mailbox != PM_SYSBOX)
{
...
if ($mailbox == PM_SYSBOX)
	$mailbox_name = $lang_messages['text_sysbox'];
}
...
if($mailbox != PM_SYSBOX)
$res = sql_query('SELECT * FROM messages WHERE receiver=' . sqlesc($CURUSER['id']) . ' AND location=' . sqlesc($mailbox) .$wherea. ' ORDER BY id DESC '.$limit) or sqlerr(__FILE__,__LINE__);
else
$res = sql_query('SELECT * FROM messages WHERE receiver=' . sqlesc($CURUSER['id']) . ' AND sender=0'.$wherea. ' ORDER BY id DESC '.$limit) or sqlerr(__FILE__,__LINE__);
}
else
{
...
<option value="-2" <?php echo ($selected == PM_SYSBOX ? " selected" : "")?>><?php echo $lang_messages['select_sysbox'] ?></option>
<?php
while ($row = mysql_fetch_assoc($res))
{
...
function messagemenu ($selected = 1) {
	...
	print ("<li" . ($selected == -2 ? " class=selected" : "") . "><a href=\"" . get_protocol_prefix() . $BASEURL . "/messages.php?action=viewmailbox&box=-2\">".$lang_messages['text_sysbox']."</a></li>");
	$res = sql_query('SELECT * FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
