...
<?php
	...
	if(isset($_GET[leavedays]) && $_GET[username]!= "" && $_GET[leavedays]!= "" && $_GET[leavedate] != ""){
		...
		for($i=0;$i<(int)$_GET[leavedays];$i++){
			...
			$Xsql="insert into signin (userid, signinleave, note, signindate) values(". $uid .", 'yes', '" . $_GET[note] ." ' , '" . $finaldate ."')";
			if(!mysql_query($Xsql)){
...
<label for="username">用&nbsp;户&nbsp;名&nbsp;：</label><input id="username" name="username" value=""/>例如：xhf<br/>
<label for="leavedate">起始日期：</label><input id="leavedate" name="leavedate" value=""/>例如：2011-11-16<br/>
