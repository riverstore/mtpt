...
$(document).ready(function(){
	...
	$("#qr").click(function(){
		...
		if($("#body").val().length < 50) err += "[简介]内容不得少于50个字符\n\n";
		if(err == "") return true;
		jAlert(err);
		return false;
	});
...
<?
	...
	"<tr><td class=rowhead align=right><b>".$lang_offers['row_title']."<font color=red>*</font></b></td><td class=rowfollow align=left><input type=text name=name id=\"name\" style=\"width: 650px;\"><br />".$lang_offers['text_title_note']."</td></tr>".
	"<tr><td class=rowhead align=right><b>".$lang_offers['row_post_or_photo']."</b></td><td class=rowfollow align=left>".
...
if ($_GET["new_offer"]){
	...
	$descrmain = unesc(RemoveXSS($_POST["body"]));
	if (!$descrmain)
	...
	if (!$arr['name']){
		...
		write_log("候选 $name 添加,操作者 ".$CURUSER[username],'normal');
...
if ($_GET["edit_offer"]) {
	...
	$s2 = "<select id=\"browsecat\" name=\"category\">\n";
...
<?
	...
	tr($lang_offers['row_title']."<font color=\"red\">*</font>", "<input type=\"text\" style=\"width: 650px\" name=\"name\" value=\"".$title."\" /><br />".$lang_offers['text_title_note'], 1);
...
if ($_GET["del_offer"]){
	...
	elseif ($sure == 1)
	{
		...
		write_log("候选: $offer ($num[name]) 被删除,操作者 $CURUSER[username]".($reason != "" ? " (".$reason.")" : ""),'normal');
...
print("<li>".get_user_class_name(1,false,true,true).$lang_offers['text_rule_one_four'].get_user_class_name($upload_class, false, true, true)."</li>\n");
...
"<b>".$lang_offers['text_offer_note1']."</b></a></div>");
print("<div align=\"center\" style=\"margin-bottom: 8px;\"><a href=\"?add_offer=1\">".
"<b>".$lang_offers['text_offer_note2']."</b></a></div>");
print("<div align=\"center\"><form method=\"get\" action=\"torrents.php?\" target=\"_blank\">".$lang_offers['text_search_offer_note'].$lang_offers['text_search_offers']."<input type=\"text\" name=\"search\">&nbsp;&nbsp;<input type=\"hidden\" name=\"incldead\" value=0>");
print("<input type=\"submit\" class=\"btn\" value=\"".$lang_offers['submit_allsearch']."\" /></form></div><br />");
...
*/
print("<div align=\"center\" style=\"margin-bottom: 8px;\"><input type=\"button\" onclick=\"window.location.href='offers.php?add_offer=1'\" value=\"".$lang_offers['text_add_offer']."\" style=\"font-size:14px;height:30px\"></div>");
end_frame();
print("<br /><br />");
