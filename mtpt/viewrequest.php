...
$allowed_actions = array("list","new","view","edit","takeedit","takeadded","res","takeres","addamount","delete","delres","fastdel","confirm","cancel","reseed","keepseed","needtran");
if (!$action)
	$action='list';
...
stderr($lang_req['std_error'], $lang_req['std_invalid_action']);
	...
	$allowed_finished = array("yes","no","cancel","all");
	if (!in_array($finished, $allowed_finished))$limit = "finish = 'no'";
	else $limit = ( $finished=="all" ? '1' : "finish ='".$finished."'");
	$rows=sql_query("SELECT count(*) FROM req WHERE ".$limit) or sqlerr(__FILE__, __LINE__);
	$rownumber = mysql_fetch_array( $rows );
	if ($rownumber[0] == 0) stderr( "没有求种" , "没有符合条件的求种项目，<a href=viewrequest.php?action=new>点击这里增加新求种</a>",0,0,0,0);
	else {
	$addparam=$oldlink;
	list($pagertop, $pagerbottom, $page) = pager($torrentsperpage, $rownumber[0], "?" . $addparam);
	
	switch($_GET['sort']) {
		case '1': $column = "catid"; break;
		case '2': $column = "name"; break;
		case '3': $column = "amount"; break;
		case '4': $column = "ori_amount"; break;
		case '5': $column = "userid"; break;
		case '6': $column = "comments"; break;
		case '7': $column = "added"; break;
		case '8': $column = "finish"; break;
		case '9': $column = "resetdate"; break;
		default: $column = "resetdate"; break;
	}
	switch($_GET['type']) {
	...
	if($order!="resetdate DESC,")$order.=" resetdate DESC";
	else $order.=" amount DESC";
	
	$rows=sql_query("SELECT * FROM req WHERE ".$limit."  ORDER BY $order $page") or sqlerr(__FILE__, __LINE__);
	print("<h1 align=center><a href=viewrequest.php?action=new><input type=\"button\" value=\"".$lang_req['head_req']."\" onclick=\"window.location='viewrequest.php?action=new';\" style=\"font-weight: bold\"/></a> <a href=forums.php?action=viewtopic&topicid=4818><input type=\"button\" value=\"规则\" onclick=\"window.location='forums.php?action=viewtopic&topicid=4770';\" style=\"color: red; font-weight: bold\"/></a></h1>");
	print($pagertop);
...
//	if
	...
	"<td align=left>{$man}<a href=viewrequest.php?action=view&id=".$row["id"]."><b>".$row["name"]."</b></a></td>".
	"<td align=center>".$row['ori_amount']."</td>".
	"<td align=center><font color=#ff0000><b>".($row['amount']-$row['ori_amount'])."</b></font></td>".
	...
	"<td align=center>".($row['finish']=="yes"?$lang_req['finished']:($row['finish']=="cancel"?"已撤消":($row['userid']== $CURUSER[id]?$lang_req['unfinished']:"<a href=viewrequest.php?action=res&id=".$row["id"]." >".$lang_req['unfinished']."</a>")))."</td>".
	(get_user_class() >= 13?"<td align=center><font color=black><a href=viewrequest.php?action=fastdel&id=".$row["id"]." >删</a> <a href=viewrequest.php?action=edit&id=".$row["id"]." >改</a> <a href=viewrequest.php?action=cancel&id=".$row["id"]." >撤</a></td>\n":"").
	...
	if (mysql_num_rows($res) == 0) stderr( $lang_req['std_error'] , $lang_req['id_not_exist']);
	...
	tr($lang_req['do'], 
	(($arr['userid']== $CURUSER[id] || get_user_class() >= 13)&&$arr["finish"]=="no" ?"<a href=viewrequest.php?action=edit&id=".$id." >".$lang_req['tr_edit']:"")."\n".//编辑权限（带分隔线）
	...
	if (mysql_num_rows($res) == 0) $ress="还没有应求";
		...
		while($row = mysql_fetch_array($res))
			...
			else $ress.="<i>种子已被删除</i>";
			$ress.=(get_user_class() >= 13?" [<a href=viewrequest.php?action=delres&id=".$row["id"]." >X</a>]":"").			
			"<br/>\n";
			}
		if(($arr['userid']== $CURUSER[id] || get_user_class() >= 13)&&$arr['finish']=="no")
	...
	if (!$count) {
		...
		list($pagertop, $pagerbottom, $limit) = pager(10, $count, "viewrequest.php?action=view&id=$id&", array(lastpagedefault => 1));
...
"<input type=\"hidden\" name=\"pid\" value=\"".$id."\" /><br />");
	...
	else stderr($lang_req['std_error'], $lang_req['id_not_exist']);
	break;
	}
	...
	if($arr['userid']== $CURUSER[id] || get_user_class() >= 13)
		...
		");
		$cat=array( 401=>"电影",402=>"剧集",403=>"综艺",404=>"资料",405=>"动漫",406=>"音乐",407=>"体育",408=>"软件",409=>"游戏",410=>"其他",411=>"纪录片");
	...
	else stderr("出错了！！！","你没有该权限！！！<a href=viewrequest.php?action=view&id=".$_GET["id"].">点击这里返回</a>",0);
	}
	...
	else stderr("出错了！！！","你没有该权限！！！<a href=viewrequest.php>点击这里返回</a>",0);
	}
	
	case "takeadded":
	{
	if(!$_POST["introduce"])stderr("出错了！","介绍未填！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!$_POST["name"])stderr("出错了！","名称未填！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!$_POST["catid"])stderr("出错了！","类型未选！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!$_POST["amount"])stderr("出错了！","赏金未填！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!is_numeric($_POST["amount"]))stderr("出错了！！！","赏金必须为数字！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	$amount=$_POST["amount"];
	if($amount<1000)stderr("出错了！","发布求种赏金不得小于1000个麦粒！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if($amount>10000)stderr("出错了！","发布求种赏金不得大于10000个麦粒！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	$amount += 100;
	if($amount+100>$CURUSER[seedbonus])stderr("出错了！","你没有那么多麦粒！！！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(get_user_class() >= 2)
		{
	...
	case "takeedit":
	{
	if(!is_numeric($_POST["reqid"]))stderr("出错了！！！","求种ID必须为数字！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	$res = sql_query("SELECT * FROM req WHERE id =".sqlesc($_POST["reqid"])."") or sqlerr(__FILE__, __LINE__);
	if(!$_POST["introduce"])stderr("出错了！！！","介绍未填！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!$_POST["name"])stderr("出错了！！！","名称未填！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if(!$_POST["catid"])stderr("出错了！！！","类型未选！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if (mysql_num_rows($res) == 0) stderr("出错了！","该求种已被删除！<a href=viewrequest.php>点击这里返回</a>",0);
	$arr = mysql_fetch_assoc($res);
	if ($arr["finish"]=="yes") stderr("出错了！","该求种已完成！<a href=viewrequest.php?action=view&id=".$_POST["reqid"].">点击这里返回</a>",0);
	if($arr['userid']== $CURUSER[id] || get_user_class() >= 13)
		{
		sql_query("UPDATE req SET introduce = ".sqlesc($_POST["introduce"])." , name = ".sqlesc($_POST["name"]).", catid = ".sqlesc($_POST["catid"])." WHERE id =".sqlesc($_POST["reqid"])."") or sqlerr(__FILE__, __LINE__);		$Cache->delete_value ( 'req_' . $_POST["reqid"] . '_req_name');
		stderr("成功","编辑成功，<a href=viewrequest.php?action=view&id=".$_POST["reqid"].">点击这里返回</a>",0);
		}
	else stderr("出错了！！！","你没有该权限！！！<a href=viewrequest.php?action=view&id=".$_POST["reqid"].">点击这里返回</a>",0);
	die;
	break;
	}
	...
	if (mysql_num_rows($res) == 0) stderr("出错了！","该求种已被删除！<a href=viewrequest.php>点击这里返回</a>",0);
	$arr = mysql_fetch_assoc($res);
	if ($arr["finish"]!="no") stderr("出错了！","该求种已完成或撤销！<a href=viewrequest.php?action=view&id=".$_POST["reqid"].">点击这里返回</a>",0);
	if(!is_numeric($_POST["torrentid"]))stderr("出错了！！！","种子ID必须为数字！<a href=viewrequest.php?action=res&id=".$_POST["reqid"].">点击这里返回</a>",0);
	$res = sql_query("SELECT * FROM torrents WHERE id =".sqlesc($_POST["torrentid"])."") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) == 0) stderr("出错了！","该种子不存在！<a href=viewrequest.php?action=res&id=".$_POST["reqid"].">点击这里返回</a>",0);
	$tor=mysql_fetch_assoc($res);
	if ($tor[last_seed] == "0000-00-00 00:00:00") stderr("出错了！！！","该种子尚未正式发布！<a href=viewrequest.php?action=res&id=".$_POST["reqid"].">点击这里返回</a>",0);
	$res = sql_query("SELECT * FROM resreq WHERE reqid=".sqlesc($_POST["reqid"])." AND torrentid=".sqlesc($_POST["torrentid"])."");
	if (mysql_num_rows($res) != 0)stderr("出错了！","请不要重复应求，<a href=viewrequest.php?action=view&id=".$_POST["reqid"].">点击这里返回</a>",0);sql_query("INSERT resreq (reqid , torrentid) VALUES ( ".sqlesc($_POST["reqid"])." , ".sqlesc($_POST["torrentid"]).")");
	...
	if($amount<100)stderr("出错了！","追加悬赏赏金不得小于100个麦粒！");
	if($amount>5000)stderr("出错了！","追加悬赏赏金不得大于5000个麦粒！");
//	$amount += 25;
	$newseedbonus = $amount+25;
	$newamount = $arr["amount"]+$amount;
	if($amount >$CURUSER[seedbonus])stderr("出错了！","你没有那么多麦粒！");
...
//		$amount -= 25;
	...
	case "delete":
	{
	if(!is_numeric($_GET["id"]))stderr("出错了！！！","求种ID必须为数字",true);
	$res = sql_query("SELECT * FROM req WHERE id ='".$_GET["id"]."'") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) == 0) stderr("出错了！","该求种已被删除！",true);
	$arr = mysql_fetch_assoc($res);
	if(get_user_class() >= 13)
		{
		if($_POST["reasontype"]){
	...
	if (is_array($ids))
	{
		foreach ($ids as $id)
		{
			if(!is_numeric($id))stderr("出错了！！！","求种ID必须为数字",true);
		}
	}
	else
	{
		if(!is_numeric($ids))stderr("出错了！！！","求种ID必须为数字",true);
		$ids = array($ids);
	}
	if(get_user_class() < 13)
	{
	...
	foreach ($ids as $id)
	{
		...
		if (!$_GET['job'] || $_GET['job'] == '删除')
		{
			...
			write_log("管理员 ".$CURUSER["username"]." 删除了求种 ".$id." ( ".$arr["name"]." )" );			$Cache->delete_value ( 'req_' . $id . '_req_name');
			if($CURUSER["id"]!=$arr['userid'])
			{
				$added = sqlesc(date("Y-m-d H:i:s"));
				$subject = sqlesc("你发布的求种".$arr["name"]."被删除");
	...
	case "delres":
	{
	if(!is_numeric($_GET["id"]))stderr("出错了！！！","非法操作I",true);
	$res = sql_query("SELECT * FROM resreq WHERE id ='".$_GET["id"]."'") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) == 0) stderr("出错了！","该应求已删除",true);
	$arr = mysql_fetch_assoc($res);
	if(get_user_class() >= 13)
		{
	...
	if (mysql_num_rows($res) == 0) stderr("出错了！","该求种已被删除！",true);
	$arr = mysql_fetch_assoc($res);
	if ($arr['finish']=="yes" ) stderr("出错了！","该求种已经完成，无法撤销！",true);
	if ($arr['finish']=="cancel" ) stderr("出错了！","该求种已经撤销！",true);
	if ($arr['userid']!= $CURUSER[id] && get_user_class() < 13) stderr("出错了！","你没有该权限！",true);
	$res = sql_query("SELECT * FROM resreq WHERE reqid ='".$_GET["id"]."'") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) > 0 && get_user_class() < 13) stderr("出错了！","	
	...
	if ($arr['added']> date("Y-m-d H:i:s",time()-14*86400) && get_user_class() < 13) stderr("出错了！","发布求种2周后才允许自行撤销！！<a href=viewrequest.php?action=view&id=".$_GET["id"].">点击这里返回</a>",false);	
	...
	case "confirm":
	{
	if(!is_numeric($_POST["id"]))stderr("出错了！！！","不要试图入侵系统",true);
	$res = sql_query("SELECT * FROM req WHERE id ='".$_POST["id"]."'") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) == 0) stderr("出错了！","该求种已被删除！",true);
	$arr = mysql_fetch_assoc($res);
	if(empty($_POST["torrentid"]))stderr("出错了！","你没有选择符合条件的应求！",true);
	else if($arr['finish']=="yes")stderr("出错了！","该应求已经确认！<a href=viewrequest.php?action=view&id=".$_POST["id"].">点击这里返回</a>",false);
	...
	if($arr['userid']== $CURUSER[id] || get_user_class() >= 13)
		...
		while($row = mysql_fetch_array($res))$owner[]=$row[0];
